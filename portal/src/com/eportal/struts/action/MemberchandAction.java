...
public class MemberchandAction extends ActionSupport implements ModelDriven<Merchandise>{
	...
	private int merId;  //物品ID
	private int merStatus;//物品状态
	private Integer messageId;//留言状态
	...
	/** 管理我的物品 */
	@SuppressWarnings("unchecked")
	public String browseMyMerchandise(){
		if(actionMsg!=null){
			try {
				actionMsg = new String(actionMsg.getBytes("ISO8859-1"),"gbk");
			} catch (UnsupportedEncodingException e) {
		...
		if (member!=null){//当前会员进行订单管理
			merList=service.browseMyMerchandise(member.getId());//调用业务逻辑组件取得商品列表
			createDoubleSelect();
		}else{//会员尚未登录或登录失效
	...
	public String addMerchandise(){
		try {
			...
			model.setSprice(model.getSprice());
	...
	public String viewMerchandise(){
		Member member = (Member)ServletActionContext.getRequest().getSession().getAttribute("member");
		if(member!=null){
		merList=service.browseMerchandiseExist(member.getId());
		}
		if (model.getId()!=null){
			//调用业务逻辑组件装载指定的商品
	...
	/**更改物品交换状态*/
	public String changeMerStatus(){
		int sts=Integer.parseInt(ServletActionContext.getRequest().getParameter("merStatus"));
		if (merId!=0){
			Merchandise mer = service.loadMerchandise(merId);
			if (mer!=null){
				if(sts==0){
					//如果为拒绝则改变留言状态
					mer.setMerStatus(0);
					boolean b=service.saveOrUpdateMerchandise(mer);
					LeaveGoodsMessage lgm=leaveService.loadGoodsMessage(messageId);
					//留言状态改为已拒绝
					lgm.setMessageExist((byte)0);
					boolean lm=leaveService.saveOrUpdateLeaveMessage(lgm);
					ChangeInfo changeInfo=lgm.getChangeInfo();
					changeInfo.setChangeState(0);
					Merchandise m=changeInfo.getGoodsInfoByChangeGoodsId();
					m.setMerStatus(0);
					service.saveOrUpdateMerchandise(m);
					boolean chan=changeinfoService.updateChange(lgm.getChangeInfo());
					if(b==true&&lm==true&&chan==true){
					try {
						...
						LeaveGoodsMessage lgm=leaveService.loadGoodsMessage(messageId);
						//留言状态改为已同意
						lgm.setMessageExist((byte)2);
						boolean lm=leaveService.saveOrUpdateLeaveMessage(lgm);
						//更新订单状态
						ChangeInfo changeInfo=lgm.getChangeInfo();
						changeInfo.setChangeState(2);
						boolean chan=changeinfoService.updateChange(lgm.getChangeInfo());
						if(b==true&&lm==true&&chan==true){
							try {
		...
		ServletActionContext.getRequest().getSession().removeAttribute("messageFlag");
	...
	public void validateAddMerchandise(){
		...
		//使用正则表达式验证商品优惠价格
			...
			if (model.getSprice()==null){
				addFieldError("sprice",getText("mer_validation_sprice"));
			}	
			if (model.getSprice().equals("七成新")||model.getSprice().equals("八成新")||model.getSprice().equals("九成新")||model.getSprice().equals("全新")){
				model.setSpecial(1);
		}		
		//商品型号必填
