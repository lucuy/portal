...
public class MerPageTag extends SimpleTagSupport{
    ...
    public void doTag() throws JspException, IOException{
    	...
    	if (listtype==1){//最新商品列表
    		if (ids!=null){//指定商品分类
    			hql = "select count(*) from Merchandise as a where a.status=1 and a.merStatus=0 and a.category.id in("+ids+")";    			
    		}else{//所有商品分类
    			hql = "select count(*) from Merchandise as a where a.status=1 and a.merStatus=0";
    		}    		
    	}else{//促销商品列表
    		if (ids!=null){//指定商品分类
    			hql = "select count(*) from Merchandise as a where a.status=1 and a.merStatus=0 and a.special=1 and a.category.id in("+ids+")";
    		}else{//所有商品分类
    			hql = "select count(*) from Merchandise as a where a.status=1 and a.merStatus=0 and a.special=1";
    		}    		
    	}
