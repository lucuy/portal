...
public class TextnewsTag extends SimpleTagSupport{
    ...
    public void doTag() throws JspException, IOException{
		...
		if (newstype.equals("4")){//所有类型
			hql=" from News as a where a.status=1  order by a.priority desc,a.id desc";			
		}else{//指定类型
			hql=" from News as a where a.status=1  and a.newsType="+newstype+" order by a.priority desc,a.id desc";			
		}
